...
if __name__ == "__main__":
    ...
    send_discord_notification(
        f"🚀 Dynamic Copy Trading started - Master: {config['MASTER_LOGIN']}, Slave: {config['SLAVE_LOGIN']}",
    ...
    if master_success and slave_success:
        print(f"\n🎯 Both terminals ready! Starting copy trading...")
        send_discord_notification(
            f"✅ Dynamic Copy Trading ready - Both terminals active!",
            config['DISCORD_WEBHOOK_URL']
        )
        ...
        try:
            ...
            send_discord_notification("⏹️ Dynamic Copy Trading stopped by user", config['DISCORD_WEBHOOK_URL'])
            print("\nCopy trading stopped by user")
            mt5.shutdown()
        except Exception as e:
            send_discord_notification(f"❌ Dynamic Copy Trading error: {str(e)}", config['DISCORD_WEBHOOK_URL'])
            print(f"Error: {e}")
            mt5.shutdown()
    else:
        print("❌ Failed to setup terminals")
        send_discord_notification("❌ Failed to setup MT5 terminals", config['DISCORD_WEBHOOK_URL'])
